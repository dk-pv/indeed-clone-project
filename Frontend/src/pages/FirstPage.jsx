...
const FirstPage = () => {
  ...
  const confirmApplyHandler = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/job/apply/${selectedJobIdToApply}`,
        {},
        {
          headers: {
  ...
  const fetchAppliedJobs = async () => {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/job/applied`, {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      if (jobQuery.trim()) {
        axios
          .get(`${import.meta.env.VITE_API_URL}/api/search/suggest/job-titles?query=${jobQuery}`)
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(() => {
      if (locationQuery.trim()) {
        axios
          .get(`${import.meta.env.VITE_API_URL}/api/search/suggest/locations?query=${locationQuery}`)
  ...
  const toggleSaveJob = async (jobId) => {
    ...
    try {
      if (isSaved) {
        ...
        await axios.post(
          `${import.meta.env.VITE_API_URL}/api/job/save/${jobId}`,
          {},
          {
            headers: { Authorization: `Bearer ${token}` },
          }
  ...
  const fetchAllJobs = async (page = 1) => {
    ...
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/job/all-jobs?page=${page}&limit=10`
      );
  ...
  const handleJobClick = async (job) => {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/job/${job._id}`);
      if (response.data.success) {
