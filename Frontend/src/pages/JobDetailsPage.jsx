  ...
  const fetchAppliesAndSaves = async () => {
    ...
    try {
      const [appliedRes, savedRes] = await Promise.all([
        axios.get(`${import.meta.env.VITE_API_URL}/api/job/applied`, {
          headers,
        }),
        axios.get(`${import.meta.env.VITE_API_URL}/api/job/saved`, { headers }),
      ]);
  ...
  const confirmApply = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/job/apply/${jobId}`,
        {},
  ...
  const toggleSave = async () => {
    try {
      ...
      if (savedJobs.includes(jobId)) {
        await axios.delete(
          `${import.meta.env.VITE_API_URL}/api/job/saved/${jobId}`,
          { headers }
        );
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 text-gray-600 mb-3">
                      ...
                      <span>
                        {jobDetails.job.location.city},{" "}
                        {jobDetails.job.location.state}
                      </span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-600">
                      <DollarSign className="w-4 h-4" />
                      <span className="font-medium text-gray-900">
                        ₹{jobDetails.payRange.min.toLocaleString()} - ₹
                        {jobDetails.payRange.max.toLocaleString()} a month
                      </span>
                    </div>
                  </div>
