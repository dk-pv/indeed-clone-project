  ...
  useEffect(() => {
    if (user) {
      ...
      if (socket && user.id && !hasJoinedRoom.current) {
        socket.emit("join", { room: user.id }); // Join socket room (employer or applicant)
        hasJoinedRoom.current = true;
        fetchNotificationCount();
      }
    } else {
              ...
              <div className="flex items-center space-x-5">
                {userRole !== "employer" ? (
                  <>
                    ...
                    <Link to="/notifications-jobseeker" className="relative p-2 text-gray-500 hover:text-blue-600 hover:bg-gray-50 rounded-full">
                      <BellDot className="h-5 w-5" />
                      {unreadCount > 0 && (
                        <span className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center">
                          {unreadCount > 9 ? "9+" : unreadCount}
                  ...
                  <>
                    ...
                    <Link to="/notifications" className="relative p-2 text-gray-500 hover:text-blue-600 hover:bg-gray-50 rounded-full">
                      <Bell className="h-5 w-5" />
                      {unreadCount > 0 && (
                        <span className="absolute top-0 right-0 bg-red-500 text-white text-xs rounded-full h-4 w-4 flex items-center justify-center">
                          {unreadCount > 9 ? "9+" : unreadCount}
                ...
                <div className="relative">
                  ...
                  {isUserDropdownOpen && (
                    <div className="absolute right-0 top-full mt-2 w-64 bg-white border rounded-lg shadow-lg z-50">
                      ...
                      <div className="py-1">
                        <Link
                          to={userRole === "employer" ? "/notifications" : "/profile"}
                          onClick={() => setIsUserDropdownOpen(false)}
                          className="flex items-center px-4 py-2 text-sm hover:text-blue-600 hover:bg-gray-50"
                        >
                          <User className="h-4 w-4 mr-3" />
                          {userRole === "employer" ? "Notifications" : "Profile"}
                          {unreadCount > 0 && (
